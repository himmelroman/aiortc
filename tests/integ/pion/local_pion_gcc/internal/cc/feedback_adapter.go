...
type FeedbackAdapter struct {
	...
	baseDepartureTime time.Time  // Track first departure to make times relative
}

// NewFeedbackAdapter returns a new FeedbackAdapter.
func NewFeedbackAdapter() *FeedbackAdapter {
	return &FeedbackAdapter{
		history:           newFeedbackHistory(5000),  // Increased from 250 to handle delayed TWCC feedback
		baseDepartureTime: time.Time{},  // Will be set on first packet
	}
}
...
func (f *FeedbackAdapter) onSentTWCC(ts time.Time, extID uint8, header *rtp.Header, size int) error {
	...
	// CRITICAL FIX: Track base departure time to make all times relative
	// This matches how arrival times are relative to reference_time (relative to epoch)
	// Start from Unix epoch + 1 second to avoid zero time for first packet
	if f.baseDepartureTime.IsZero() {
		f.baseDepartureTime = ts
		log.Printf("🔧 DEBUG OnSent: Set base departure time")
	}

	// Store departure time relative to base, starting from epoch + 1s to avoid zero time
	...
	ack := Acknowledgment{
		...
		Departure:      relativeDeparture,  // Now relative, matching arrival time coordinate system
		Arrival:        time.Time{},
		ECN:            0,
	}
	f.history.add(ack)
	...
	if tccExt.TransportSequence%100 == 0 {
		log.Printf("🔧 DEBUG OnSent: Recorded TWCC seq=%d, Departure=%v (size now: %d)", tccExt.TransportSequence, ack.Departure, len(f.history.items))
	}
...
func (f *FeedbackAdapter) unpackRunLengthChunk(
	...
	for i := start; i != end; i++ {
		...
		if ack, ok := f.history.get(key); ok {
			foundCount++
			if foundCount <= 3 && start == 0 {
				log.Printf("🔧 DEBUG history.get: seq=%d, Departure=%v, Arrival=%v", i, ack.Departure, ack.Arrival)
			}
	...
	if start%100 == 0 {
		log.Printf("🔧 DEBUG unpackRunLengthChunk: start=%d, runLength=%d, found=%d in history", start, chunk.RunLength, foundCount)
	}
...
func (f *FeedbackAdapter) OnTransportCCFeedback(
	...
	log.Printf("🔧 DEBUG OnTransportCCFeedback: base_seq=%d, history_size=%d, created %d acks",
		feedback.BaseSequenceNumber, len(f.history.items), len(result))
