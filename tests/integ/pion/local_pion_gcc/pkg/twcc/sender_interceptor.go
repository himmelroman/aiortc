...
func (s *SenderInterceptor) loop(writer interceptor.RTCPWriter) {
	...
	twccDumpPath := "/tmp/pion_twcc_packets.bin"
	twccDump, err := os.Create(twccDumpPath)
	if err != nil {
		s.log.Warnf("Could not open TWCC dump file: %v", err)
	} else {
		s.log.Infof("📝 TWCC packet dump: %s", twccDumpPath)
		defer func() {
			if twccDump != nil {
				twccDump.Close()
				s.log.Infof("📝 TWCC packet dump closed")
			}
		}()
	}
	...
	for {
		...
		case <-ticker.C:
			...
			// Dump packets to file for analysis
			if twccDump != nil {
				for _, pkt := range pkts {
					data, marshalErr := pkt.Marshal()
					if marshalErr != nil {
						s.log.Warnf("Failed to marshal TWCC packet: %v", marshalErr)
						continue
					}

					// Write packet length (4 bytes big-endian) then packet data
					lengthBytes := []byte{
						byte(len(data) >> 24),
						byte(len(data) >> 16),
						byte(len(data) >> 8),
					...
					if _, writeErr := twccDump.Write(lengthBytes); writeErr != nil {
						s.log.Warnf("Failed to write TWCC packet length: %v", writeErr)
					} else if _, writeErr := twccDump.Write(data); writeErr != nil {
						s.log.Warnf("Failed to write TWCC packet data: %v", writeErr)
					} else {
						twccDump.Sync() // Flush to disk
						s.log.Infof("📡 TWCC: Dumped feedback packet (%d bytes)", len(data))
					}
				}
			}
